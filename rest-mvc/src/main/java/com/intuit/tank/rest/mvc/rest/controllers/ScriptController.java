...
public class ScriptController {
    ...
    @Operation(description = "Uploads a script XML file output generated by the Tank Proxy Package (supports gzip files) \n\n " +
                             "Examples: \n\n" +
                             "        curl -v -X POST -H 'Content-Type: multipart/form-data' -F \"file=@<filename>\" 'https://{tank-base-url}/api/v2/scripts/upload' \n\n" +
                             "\n\n" +
                             "        curl -v -X POST -H 'Content-Type: multipart/form-data' -F \"file=@<filename>\" 'https://{tank-base-url}/api/v2/scripts/upload?id=<scriptId>&name=<script-name>'\n\n" +
                             "\n\n" +
                             "        gzip <filename>\n\n" +
                             "        curl -v -X POST -H 'Content-Type: multipart/form-data' -H 'Content-Encoding: gzip' -F \"file=@<filename>.gz\" 'https://{tank-base-url}/api/v2/scripts/upload'\n\n" +
                             "Notes: \n\n " +
