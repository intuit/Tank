...
/**
 * A strategy for painting the background of an <code>RTextAreaBase</code> as an image. The image is always stretched to
 * completely fill the <code>RTextAreaBase</code>.
 * <p>
 * 
 * You can set the scaling hint used when stretching/skewing the image to fit in the <code>RTextAreaBase</code>'s
 * background via the <code>setScalingHint</code> method, but keep in mind the more accurate the scaling hint, the less
 * responsive your application will be when stretching the window (as that's the only time the image's size is
 * recalculated).
 * 
...
public abstract class ImageBackgroundPainterStrategy
    ...
    /**
     * Returns the "master" image; that is, the original, unscaled image. When the image needs to be rescaled, scaling
     * should be done from this image, to prevent repeated scaling from distorting the image.
     * 
     ...
     * @param bounds
     *            The bounds in which to paint the image. The image will be scaled to fit exactly in these bounds if
     *            necessary.
     */
    ...
    /**
     * Paints the image at the specified location. This method assumes scaling has already been done, and simply paints
