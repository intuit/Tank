...
class TipWindow extends JWindow implements ActionListener {
    ...
    /**
     * Workaround for JEditorPane not returning its proper preferred size when rendering HTML until after layout already
     * done. See http://forums.sun.com/thread.jspa?forumID=57&threadID=574810 for a discussion.
     */
    void fixSize() {
     ...
     * 
     * @param listener
     *            The new listener. The old listener (if any) is removed. A value of <code>null</code> means
     *            "no listener."
     */
