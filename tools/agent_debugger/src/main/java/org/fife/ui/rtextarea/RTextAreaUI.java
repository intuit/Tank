...
/**
 * The UI used by instances of <code>RTextArea</code>. This UI takes into account all of the "extras" involved in an
 * <code>RTextArea</code>, including having a special caret (for insert and overwrite), background images, highlighting
...
public class RTextAreaUI extends BasicTextAreaUI implements ViewFactory {
     ...
     *             If <code>textArea</code> is not an instance of <code>RTextArea</code>.
     */
    ...
    /**
     * The Nimbus LAF (and any Synth laf might have similar issues) doesn't set many UIManager properties that BasicLAF
     * UI's look for. This causes problems for custom Basic-based UI's such as RTextAreaUI. This method attempts to
     * detect if Nimbus has been installed, and if so, sets proper values for some editor properties.
     * 
     * @param editor
     *            The text area.
     */
    ...
    /**
     * Returns the default caret for an <code>RTextArea</code>. This caret is capable of displaying itself differently
     * for insert/overwrite modes.
     * 
     * @return The caret.
     */
    ...
    /**
     * Creates the keymap for this text area. This takes the super class's keymap, but sets the default keystroke to be
     * RTextAreaEditorKit's DefaultKeyTypedAction. This must be done to override the default keymap's default key-typed
     * action.
     * 
     * @return The keymap.
     */
    ...
    /**
     * Creates a default action map. This action map contains actions for all basic text area work - cut, copy, paste,
     * select, caret motion, etc.
     * <p>
     * 
     * This isn't named <code>createActionMap()</code> because there is a package-private member by that name in
     * <code>BasicTextAreaUI</code>, and some compilers will give warnings that we are not overriding that method since
     ...
     * <p>
     * 
     * This method is not named <code>getActionMap()</code> because there is a package-private method in
    ...
    /**
     * Gets the allocation to give the root View. Due to an unfortunate set of historical events this method is
     * inappropriately named. The Rectangle returned has nothing to do with visibility. The component must have a
     ...
     * </pre>
     * 
     * Subclasses that can calculate this value more quickly than traditional {@link #modelToView(JTextComponent, int)}
     * calls should override this method to do so. This method may be used when the entire bounding box isn't needed, to
     * speed up rendering.
     * 
     * @param offs
