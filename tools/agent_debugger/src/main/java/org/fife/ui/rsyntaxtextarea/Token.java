 ...
 * <p>
 * 
 * A <code>Token</code> is a piece of text representing some logical token in source code for a programming language.
 ...
 * </pre>
 * 
 * would be broken into 8 <code>Token</code>s: the first representing <code>int</code>, the second whitespace, the third
 * <code>i</code>, the fourth whitespace, the fifth <code>=</code>, etc.
 * <p>
 * 
...
public abstract class Token {
     ...
     * @param fontFamily
     *            Whether to include the font family in the HTML for this token. You can pass <code>false</code> for
    ...
    /**
     * Appends an HTML version of the lexeme of this token (i.e. no style HTML, but replacing chars such as
     * <code>\t</code>, <code>&lt;</code> and <code>&gt;</code> with their escapes).
     * 
     * @param sb
     *            The buffer to append to.
     ...
     * <p>
     * Note that this method does NOT do any bounds checking; you can pass in a document position that does not
     * correspond to a position in this token, and you will not receive an Exception or any other notification; it is up
     * to the caller to ensure valid input.
     * 
     * @param pos
     *            A position in the document that is represented by this token.
     * @return The corresponding token position >= <code>textOffset</code> and < <code>textOffset+textCount</code>.
     * @see #tokenToDocument
     */
    ...
    /**
     * Returns a <code>String</code> of the form "#xxxxxx" good for use in HTML, representing the given color.
     * 
     * @param color
     *            The color to get a string for.
     * @return The HTML form of the color. If <code>color</code> is <code>null</code>, <code>#000000</code> is returned.
     */
    ...
    /**
     * Returns a <code>String</code> containing HTML code for painting this token, using the given text area's color
     * scheme.
     * 
     * @param textArea
     *            The text area whose color scheme to use.
    ...
    /**
     * Returns the last paintable token in this token list, or <code>null</code> if there is no paintable token.
     * 
     ...
     * <p>
     * 
     * Note that this method isn't used much by the <code>rsyntaxtextarea</code> package internally, as it tries to
    ...
    /**
     * Determines the offset into this token list (i.e., into the document) that covers pixel location <code>x</code> if
     * the token list starts at pixel location <code>x0</code>
     * <p>
     * . This method will return the document position "closest" to the x-coordinate (i.e., if they click on the
     * "right-half" of the <code>w</code> in <code>awe</code>, the caret will be placed in between the <code>w</code>
     * and <code>e</code>; similarly, clicking on the left-half places the caret between the <code>a</code> and
     * <code>w</code>). This makes it useful for methods such as <code>viewToModel</code> found in
     * <code>javax.swing.text.View</code> subclasses.
     * <p>
     * 
     * This method is abstract so subclasses who paint themselves differently (i.e., {@link VisibleWhitespaceToken} is
     * painted a tad differently than {@link DefaultToken} when rendering hints are enabled) can still return accurate
     * results.
     * 
     * @param textArea
     ...
     *            How to expand tabs.
     * @param x0
     *            The pixel x-location that is the beginning of <code>tokenList</code>.
     * @param x
     ...
     *         <code>tokenList</code> is <code>null</code> or has type <code>Token.NULL</code>, then <code>-1</code is
     *         returned; the caller should recognize this and return the actual end position of the (empty) line.
     */
     ...
     * @param e
     *            Describes how to expand tabs. This parameter cannot be <code>null</code>.
     * @param x0
     *            The pixel-location at which the token begins. This is needed because of tabs.
    ...
    /**
     * Returns whether or not this token is "paintable;" i.e., whether or not the type of this token is one such that it
     * has an associated syntax style. What this boils down to is whether the token type is greater than
     ...
     * if it isn't, <code>null</code> is returned.
     * 
     * @param textArea
     *            The text area from which the token list was derived.
     * @param e
     ...
     * 
     * @param pos
     *            The offset into the document this token should start at. Note that this token must already contain
     *            this position; if it doesn't, an exception is thrown.
     ...
     * 
     * @param amt
     *            The amount to move the starting offset. This should be between <code>0</code> and
     ...
     *            us to not paint paint when this token is "to the left" of the clip rectangle.
     * @return The x-coordinate representing the end of the painted text.
     */
     ...
     * @param width
     *            The width of the token (actually, the width of the part of the token to paint).
     * @param height
    ...
    /**
     * Returns the position in the document corresponding to the specified position in this token's internal char array
     * (<code>textOffset</code> - <code>textOffset+textCount-1</code>).
     * <p>
     * Note that this method does NOT do any bounds checking; you can pass in an invalid token position, and you will
     * not receive an Exception or any other indication that the returned document position is invalid. It is up to the
     * user to ensure valid input.
     * 
     * @param pos
     *            A position in the token's internal char array (<code>textOffset</code> -
