...
class IconRowHeader extends AbstractGutterComponent implements MouseListener {
    ...
    /**
     * @return the currentLineIcon
     */
    public Icon getCurrentLineIcon() {
        return currentLineIcon;
    }

    /**
     * @param currentLineIcon
     *            the currentLineIcon to set
     */
    public void setCurrentLineIcon(Icon currentLineIcon) {
        this.currentLineIcon = currentLineIcon;
    }

    /**
     * @return the trackingIcons
     */
    @SuppressWarnings("unchecked")
    public GutterIconInfo[] getAllTrackingIcons() {
        List<GutterIconImpl> retVal = new ArrayList<GutterIconImpl>();
        if (trackingIcons != null) {
            retVal = IntStream.range(0, trackingIcons.size()).mapToObj(this::getTrackingIcon).collect(Collectors.toList());
        }
    ...
    public GutterIconInfo[] getBookmarks() {
        ...
        List<GutterIconImpl> retVal = new ArrayList<GutterIconImpl>();
        if (trackingIcons != null) {
            retVal = IntStream.range(0, trackingIcons.size()).mapToObj(this::getTrackingIcon).filter(ti -> ti.getIcon() == bookmarkIcon).collect(Collectors.toList());
        }
        Collections.sort(retVal);
        GutterIconInfo[] array = new GutterIconInfo[retVal.size()];
        return (GutterIconInfo[]) retVal.toArray(array);
     ...
     * @param line
     *            The line.
     * @return The tracking icons at that line. If there are no tracking icons there, this will be an empty array.
     * @throws BadLocationException
    ...
    private void paintComponentWrapped(Graphics g) {
        ...
        while (y < visibleBottom) {
             ...
             * topLine==currentLine) { g.setColor(textArea.getCurrentLineHighlightColor()); g.fillRect(0,y,
     ...
     * 
     * @param enabled
     *            Whether bookmarking is enabled. If this is <code>false</code>, any bookmark icons are removed.
     * @see #isBookmarkingEnabled()
     * @see #setBookmarkIcon(Icon)
     */
     ...
     */
    public boolean setCurrentLine(int line) throws BadLocationException {
        return false;
    }
    ...
    /**
     * Sets our background color to that of standard "panels" in this LookAndFeel. This is necessary because, otherwise,
     * we'd inherit the background color of our parent component (the Gutter).
     */
