...
/**
 * A token maker that turns text into a linked list of <code>Token</code>s for syntax highlighting UNIX shell scripts.
 * 
    ...
    public Token getTokenList(Segment text, int startTokenType, final int startOffset) {
        ...
        for (int i = offset; i < end; i++) {
            ...
            case Token.LITERAL_BACKQUOTE:
                ...
                case '`':
                    if (!backslash) {
                        addToken(text, currentTokenStart, i, Token.LITERAL_BACKQUOTE, newStartOffset
                                + currentTokenStart);
                        currentTokenType = Token.NULL;
                        // backslash is definitely false when we leave.
                        break;
                ...
                case '$':
                    ...
                    addToken(text, currentTokenStart, i - 1, Token.LITERAL_BACKQUOTE, newStartOffset
                            + currentTokenStart);
                    currentTokenType = Token.VARIABLE;
                    currentTokenStart = i;
                    ...
                    if (i < end - 1 && array[i + 1] == '{') {
                        ...
                        if (i == end) { // Happens when '}' wasn't found...
                            addToken(text, currentTokenStart, end - 1, Token.VARIABLE, newStartOffset
                                    + currentTokenStart);
            ...
            case Token.LITERAL_STRING_DOUBLE_QUOTE:
                ...
                case '$':
                    ...
                    // Add the string up-to the variable.
                    addToken(text, currentTokenStart, i - 1, Token.LITERAL_STRING_DOUBLE_QUOTE, newStartOffset
                            + currentTokenStart);
                    currentTokenType = Token.VARIABLE;
                    currentTokenStart = i;
