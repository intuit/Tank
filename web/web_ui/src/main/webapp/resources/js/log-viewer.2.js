(function(window, $) {
    ...
    LogViewer.prototype.cacheDom = function() {
        ...
        this.volumeAxis = root.querySelector('[data-role="volume-axis"]');
        this.volumeStart = root.querySelector('[data-role="volume-start"]');
        this.volumeSpan = root.querySelector('[data-role="volume-span"]');
        this.volumeEnd = root.querySelector('[data-role="volume-end"]');
    ...
    LogViewer.prototype.createRow = function(event, eventIndex, visiblePos) {
        ...
        var previous = visiblePos > 0 ? viewer.events[viewer.visibleIndexes[visiblePos - 1]] : null;
        var timeCell = document.createElement("span");
        timeCell.className = "log-entry-time";
        var fullTime = event.timestampDisplay || event.timestamp || "";
        timeCell.title = fullTime || "No timestamp parsed for this line";
        var clock = document.createElement("span");
        clock.className = "log-entry-clock";
        var shortTime = P.shortTimestamp(event.timestamp) || "—";
        if (previous && P.shortDate(previous.timestamp) !== P.shortDate(event.timestamp)
                && P.shortDate(event.timestamp)) {
            shortTime = P.shortDate(event.timestamp) + " " + shortTime;
        }
        clock.textContent = shortTime;
        timeCell.appendChild(clock);
        var prevMs = P.eventEpochMs(previous);
        var thisMs = P.eventEpochMs(event);
        if (Number.isFinite(prevMs) && Number.isFinite(thisMs)) {
            var gap = thisMs - prevMs;
            var delta = document.createElement("span");
            delta.className = "log-entry-delta" + (gap >= 60000 ? " is-large" : "");
            delta.textContent = gap === 0 ? "same ms" : P.formatDuration(gap);
            delta.title = gap === 0
                    ? "Same millisecond as the previous visible event"
                    : "Gap since previous visible event";
            timeCell.appendChild(delta);
        }
    ...
    LogViewer.prototype.renderVolume = function() {
        ...
        var volume = P.volumeBuckets(visibleEvents, 32);
        var buckets = volume.buckets || [];
        this.volumeElement.replaceChildren();
        if (this.volumeAxis) {
            if (!volume.startMs || !volume.endMs) {
                this.volumeAxis.hidden = true;
            } else {
                this.volumeAxis.hidden = false;
                var startIso = new Date(volume.startMs).toISOString();
                var endIso = new Date(volume.endMs).toISOString();
                this.volumeStart.textContent = P.shortTimestamp(startIso);
                this.volumeStart.title = P.displayTimestamp(startIso);
                this.volumeEnd.textContent = P.shortTimestamp(endIso);
                this.volumeEnd.title = P.displayTimestamp(endIso);
                this.volumeSpan.textContent = volume.spanMs < 1000
                        ? "span <1s"
                        : "span " + P.formatDuration(volume.spanMs).replace(/^\+/, "");
                this.volumeSpan.title = "Visible events cover "
                        + P.displayTimestamp(startIso) + " → " + P.displayTimestamp(endIso);
            }
        }
        ...
        buckets.forEach(function(bucket) {
            ...
            var height = Math.max(bucket.total ? 4 : 2, Math.round((bucket.total / max) * 100));
            ...
            if (bucket.startMs && bucket.endMs) {
                var from = P.shortTimestamp(new Date(bucket.startMs).toISOString());
                var to = P.shortTimestamp(new Date(bucket.endMs).toISOString());
                bar.title = bucket.total + " event" + (bucket.total === 1 ? "" : "s")
                        + " · " + from + " → " + to
                        + (bucket.error ? " · " + bucket.error + " error" : "")
                        + (bucket.warn ? " · " + bucket.warn + " warn" : "");
            } else {
