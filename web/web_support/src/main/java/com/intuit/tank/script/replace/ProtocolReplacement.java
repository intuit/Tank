...
public class ProtocolReplacement extends AbstractReplacement {

    public ProtocolReplacement() {
        super(RequestStepSection.protocol, ScriptConstants.REQUEST);
    }

    @Override
    public List<ReplaceEntity> getReplacements(ScriptStep step, String searchQuery, String replaceString,
            SearchMode searchMode) {
        return getReplacementInValue(searchQuery, replaceString, step.getProtocol(), step.getType());
    }

    @Override
    public void replace(ScriptStep step, String replaceString, String key, ReplaceMode replaceMode) {
        step.setProtocol(replaceString);
    }
