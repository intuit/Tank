...
public class NameReplacement extends AbstractReplacement {

    public NameReplacement() {
        super(RequestStepSection.name, ScriptConstants.REQUEST);
    }

    @Override
    public List<ReplaceEntity> getReplacements(ScriptStep step, String searchQuery, String replaceString,
            SearchMode searchMode) {
        return getReplacementInValue(searchQuery, replaceString, step.getName(), step.getType());
    }

    @Override
    public void replace(ScriptStep step, String replaceString, String key, ReplaceMode replaceMode) {
        step.setName(replaceString);
    }
}
