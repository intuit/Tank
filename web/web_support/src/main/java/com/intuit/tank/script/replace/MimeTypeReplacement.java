...
public class MimeTypeReplacement extends AbstractReplacement {

    public MimeTypeReplacement() {
        super(RequestStepSection.mimeType, ScriptConstants.REQUEST);
    }

    @Override
    public List<ReplaceEntity> getReplacements(ScriptStep step, String searchQuery, String replaceString,
            SearchMode searchMode) {
        return getReplacementInValue(searchQuery, replaceString, step.getMimetype(), step.getType());
    }

    @Override
    public void replace(ScriptStep step, String replaceString, String key, ReplaceMode replaceMode) {
        step.setMimetype(replaceString);
    }
