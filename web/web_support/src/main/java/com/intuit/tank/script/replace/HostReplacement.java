...
public class HostReplacement extends AbstractReplacement {

    public HostReplacement() {
        super(RequestStepSection.host, ScriptConstants.REQUEST);
    }

    @Override
    public List<ReplaceEntity> getReplacements(ScriptStep step, String searchQuery, String replaceString,
            SearchMode searchMode) {
        return getReplacementInValue(searchQuery, replaceString, step.getHostname(), step.getType());
    }

    @Override
    public void replace(ScriptStep step, String replaceString, String key, ReplaceMode replaceMode) {
        step.setHostname(replaceString);
    }
