...
public class TankOkHttpClientTest {
	...
	public void addAuth() {
		BaseRequest request = getRequest(new TankOkHttpClient(),
				"http://httpbin.org/basic-auth/test/test_pass");
		request.getHttpclient().addAuth(
				AuthCredentials.builder().withUserName("test")
						.withPassword("test_pass").withRealm("bogus")
						.withScheme(AuthScheme.BASIC).build());
		...
		request.getHttpclient().addAuth(
				AuthCredentials.builder().withUserName("test")
						.withPassword("test_pass").withRealm("Fake Realm")
						.withScheme(AuthScheme.BASIC).build());
		request.doGet(null);
	...
	public void doPut() {
		...
		Assert.assertEquals(true,
				response.getResponseBody().contains("helloKey"));
		Assert.assertEquals(200, response.getHttpCode());
	}
	...
	public void clearSession() {
		...
		request.getHttpclient().setCookie(
				TankCookie.builder().withName("test-cookie")
						.withValue("test-value").withDomain("httpbin.org")
						.withPath("/").build());
		request.doGet(null);
		BaseResponse response = request.getResponse();
		Assert.assertNotNull(response);
	...
	public void setCookie() {
		...
		request.getHttpclient().setCookie(
				TankCookie.builder().withName("test-cookie")
						...
						.withPath("/").build());
		request.getHttpclient().setCookie(
				TankCookie.builder().withName("test-cookie1")
						.withValue("test-value1").withDomain("http2bin.org")
						.withPath("/").build());
		request.doGet(null);
		BaseResponse response = request.getResponse();
		...
		request = getRequest(new TankOkHttpClient(),
				"http://http2bin.org/cookies");
		request.getHttpclient().setCookie(
				TankCookie.builder().withName("test-cookie")
						.withValue("test-value").withDomain("httpbin.org")
						.withPath("/").build());
	...
	public void setHeader() {
		...
		Assert.assertTrue(request.getHeaderInformation().containsKey(
				"addRequestHeader"));
		Assert.assertTrue(request.getHeaderInformation().containsValue(
				"RequestHeader"));
		...
		Assert.assertTrue(response.getHttpHeader("OkHttp_Respone_Header")
				.contains("Set"));
	}
...
//	}
	...
	private String createMultiPartBody() throws IOException {
		...
		multipartBuilder.addFormDataPart("textPart", null, RequestBody.create(
				MediaType.parse("application/atom+xml"),
				"<xml>here is sample xml</xml>"));
