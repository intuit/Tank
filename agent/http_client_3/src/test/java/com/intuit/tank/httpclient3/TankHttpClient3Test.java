...
public class TankHttpClient3Test {
    ...
    public void doGetException() throws IOException {
        ...
        when(mockHttpClient
                .executeMethod(ArgumentMatchers.any(HttpMethod.class)))
                .thenThrow(new UnknownHostException("Mocked UnknownHost exception"));
        BaseRequest request = getRequest(new TankHttpClient3(mockHttpClient), wireMockServer.baseUrl() + "/get");
        request.doGet(null);
    ...
    public void doOptions() {
        BaseRequest request = getRequest(new TankHttpClient3(), wireMockServer.baseUrl() + "/options");
...
//        assertEquals(response.getHttpCode(), 403);
    ...
    @Test
    @Tag(TestGroups.FUNCTIONAL)
    public void doPutMultipartwithFile() throws IOException {
        BaseRequest request = getRequest(new TankHttpClient3(), wireMockServer.baseUrl() + "/put");
        request.setContentType(BaseRequest.CONTENT_TYPE_MULTIPART);
        request.setBody(
                "LS0tLS0tLS0tLS0tLS0tLS0tLS0tLS0tLS0tLS0xNzI2MTE1MzQ5Mjk4MjYNCkNvbnRlbnQtRGlzcG9zaXRpb246IGZvcm0tZGF0YTsgbmFtZT0iY3JlYXRlTmV3"
